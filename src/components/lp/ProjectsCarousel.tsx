...
export default function ProjectsCarousel({ slides, autoAdvanceMs = 0 }: ProjectsCarouselProps) {
  ...
  if (!slides || slides.length === 0) {
    return (
      <div className="p-8 rounded-2xl border border-white/10 bg-white/5 text-sm text-white/70">
  ...
  useEffect(() => {
    if (!autoAdvanceMs || autoAdvanceMs < 2500) return;
    const id = setTimeout(next, autoAdvanceMs);
    return () => clearTimeout(id);
  }, [idx, autoAdvanceMs, next]);
  ...
  // Derive highlight segment: attempt to split on dash or " – "
  ...
  const rating = typeof s.rating === 'number' ? s.rating : 4.9; // fallback visual
  const awards = typeof s.awards === 'number' ? s.awards : 5; // fallback
            ...
            <div className="max-w-md">
              ...
              <div className="mt-4 flex items-center gap-5 flex-wrap text-white/80 text-xs sm:text-sm">
                ...
                <div className="flex items-center gap-1 text-white/60 group-hover:text-white/70 transition-colors">
                  <span className="text-[11px] tracking-wide uppercase">{idx + 1}/{slides.length}</span>
                </div>
              </div>
